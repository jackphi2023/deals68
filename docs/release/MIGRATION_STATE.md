...
| Pending | `20260724163000_public_business_view_band_helper_acl_fix_v1.sql` — apply after QA; reconcile to the production-assigned version immediately |
...
- `20260724163000_public_business_view_band_helper_acl_fix_v1.sql` — additive public Business availability fix. It restores EXECUTE for anon/authenticated on four immutable, table-free coarse-band helpers required by `public_businesses_safe`; it does not reopen `businesses`, alter RLS, or expose exact financial values.
