...
- `main` remained at `641030f9c8069c0519d052310896017a171bb0ec` throughout Phase E.
- Phase C and D were integrated into `building`; the Phase E source baseline is `c7a73a77b91caba738f0aa9b062dd66b3580e377`.
...
## Production-schema rollback validation

The complete migration was executed against the current production schema inside one PostgreSQL transaction with the final statement changed to `ROLLBACK`.

The validation also:

- set a transaction-local service-role claim;
- invoked the new audited file-access RPC using an existing approved file;
- invoked the hardened file-metadata RPC for the matching Business;
- compiled the replacement table and Storage policies.

Result: the migration compiled and both RPC calls completed inside the transaction.
