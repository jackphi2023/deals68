...
function svgData(
  ...
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}">
      <defs>
        <linearGradient id="g" x1="0" x2="1"><stop stop-color="${from}"/><stop offset="1" stop-color="${to}"/></linearGradient>
      </defs>
...
const investors = Array.from({ length: 4 }, (_, index) => ({
  ...
  stage: 'growth',
  ticket_min: 250000,
  ticket_max: 3000000 + index * 1000000,
  criteria: {
  ...
  home_hero: [
    {
      id: 'h0-hero-1',
      placement: 'home_hero',
      title: 'H0 deterministic Hero',
      image_url: heroDesktop,
      mobile_image_url: heroMobile,
      focal_x: 50,
      focal_y: 50,
      mobile_focal_x: 50,
      mobile_focal_y: 50,
  ...
  home_promotion: [
    {
      ...
      title: 'H0 deterministic Promotion',
      image_url: promotion,
...
async function installDeterministicApi(page: Page) {
  ...
  await page.route('https://h0-baseline.supabase.co/**', async (route) => {
    ...
    if (url.pathname.endsWith('/rest/v1/rpc/get_homepage_business_ids')) {
      return json(route, []);
    }
...
test('capture Homepage H0 source-equivalent baseline', async ({ page }) => {
  ...
  await page.screenshot({
    path: path.join(outputDir, 'home-en-1440x900.png'),
    fullPage: true,
    animations: 'disabled',
  });
  ...
  const computed = await page.evaluate(() => {
    ...
    const promotion = document.querySelector<HTMLElement>('.d68-promo-banner');
    const directChildren = pageNode
      ? Array.from(pageNode.children).filter((node) =>
          node.matches('.d68-home-hero, .d68-home-block'),
        ) as HTMLElement[]
      : [];
    ...
    const sections = directChildren.map((node, index) => {
      ...
      const result = {
        ...
        visualGapFromPrevious:
          previousBottom === null
            ? null
            : Math.round(rect.top + window.scrollY - previousBottom),
      };
    ...
    const containers = Array.from(
      document.querySelectorAll<HTMLElement>('.d68-home-container'),
    ).map((node) => {
      const rect = node.getBoundingClientRect();
      return {
        className: node.className,
    ...
    const promotionStyle = promotion ? getComputedStyle(promotion) : null;
    ...
    return {
      ...
      page: pageStyle
        ? {
            ...
            flexDirection: pageStyle.flexDirection,
      ...
      hero: heroStyle
        ? {
            ...
            aspectRatio: heroStyle.aspectRatio,
          }
        : null,
      ...
      promotion: promotionStyle
        ? {
            paddingTop: promotionStyle.paddingTop,
            paddingBottom: promotionStyle.paddingBottom,
            marginTop: promotionStyle.marginTop,
            marginBottom: promotionStyle.marginBottom,
          }
        : null,
      sections,
      containers,
  ...
  fs.writeFileSync(
    path.join(outputDir, 'computed-styles.json'),
    `${JSON.stringify({ generatedAt: new Date().toISOString(), captures, computed }, null, 2)}\n`,
    'utf8',
  );
