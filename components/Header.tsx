...
export default function Header({ lang }: { lang: Lang }) {
  ...
  return <header style={{ position: 'sticky', top: 0, zIndex, background: 'rgba(255,255,255,.94)', backdropFilter: 'blur(10px)', borderBottom: '1px solid #E7EDF3' }}>
    <div style={{ maxWidth, margin: '0 auto', padding: '0 24px', height: 70, display: 'flex', alignItems: 'center', gap: 28 }}>
      ...
      <button
        type="button"
        className="d68-burger"
        aria-label={drawerOpen ? T(lang, 'Đóng menu', 'Close menu') : T(lang, 'Mở menu', 'Open menu')}
        aria-expanded={drawerOpen}
        onClick={() => setDrawerOpen((v) => !v)}
        style={{ marginLeft: 'auto', width: 44, height: 44, alignItems: 'center', justifyContent: 'center', border: '1px solid #E2E8F0', borderRadius: 10, fontSize: 22, color: '#14315A', cursor: 'pointer', flexShrink: 0, background: '#fff' }}
      >{drawerOpen ? '×' : '☰'}</button>

      <div className="d68-mdrawer" style={{ display: drawerOpen ? 'flex' : 'none', position: 'absolute', top: 70, left: 0, right: 0, flexDirection: 'column', gap: 2, background: '#fff', borderTop: '1px solid #E7EDF3', boxShadow: '0 18px 34px rgba(15,42,74,.14)', padding: '12px 20px 18px', maxHeight: 'calc(100vh - 70px)', overflowY: 'auto' }}>
        <Link onClick={closeDrawer} to={nav('/businesses')} style={{ padding: '13px 6px', fontSize: 16, fontWeight: 600, color: '#14315A', borderBottom: '1px solid #F1F5F9' }}><span className="l-vi">Doanh nghiệp</span><span className="l-en">Businesses</span></Link>
        <Link onClick={closeDrawer} to={nav('/investors')} style={{ padding: '13px 6px', fontSize: 16, fontWeight: 600, color: '#14315A', borderBottom: '1px solid #F1F5F9' }}><span className="l-vi">Nhà đầu tư</span><span className="l-en">Investors</span></Link>
        <Link onClick={closeDrawer} to={nav('/valuation')} style={{ padding: '13px 6px', fontSize: 16, fontWeight: 600, color: '#14315A', borderBottom: '1px solid #F1F5F9' }}><span className="l-vi">Định giá</span><span className="l-en">Valuation</span></Link>
        <Link onClick={closeDrawer} to={nav('/pricing')} style={{ padding: '13px 6px', fontSize: 16, fontWeight: 600, color: '#14315A', borderBottom: '1px solid #F1F5F9' }}><span className="l-vi">Bảng giá</span><span className="l-en">Pricing</span></Link>
        ...
        {profile
          ...
          : <>
              <Link onClick={closeDrawer} to={nav('/login')} style={{ padding: '13px 6px', fontSize: 16, fontWeight: 700, color: '#14315A' }}><span className="l-vi">Đăng nhập</span><span className="l-en">Log in</span></Link>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 8 }}>
                <Link onClick={closeDrawer} to={nav('/register/business')} style={{ textAlign: 'center', background: '#1BADEA', color: '#fff', fontWeight: 700, fontSize: 15, padding: 13, borderRadius: 10 }}><span className="l-vi">Đăng ký Doanh nghiệp</span><span className="l-en">Register as Business</span></Link>
                <Link onClick={closeDrawer} to={nav('/register/investor')} style={{ textAlign: 'center', border: '1px solid #E2E8F0', color: '#14315A', fontWeight: 700, fontSize: 15, padding: 13, borderRadius: 10 }}><span className="l-vi">Đăng ký Nhà đầu tư</span><span className="l-en">Register as Investor</span></Link>
              </div>
            </>}
      </div>
