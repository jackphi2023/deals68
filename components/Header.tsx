...
import { localizedPath, switchLangPath } from '../lib/i18nRoutes';
...
export default function Header({ lang, setLang }: { lang: Lang; setLang: (l: Lang) => void }) {
  ...
  const maxWidth = pathname === '/' || pathname === '/en' ? 1200 : 1240;
  const zIndex = pathname.includes('/businesses') || pathname.includes('/investors') ? 60 : 50;
  ...
  const switchLang = (target: Lang) => {
    setLang(target);
    const next = switchLangPath(pathname, search, hash, target);
    if (next !== `${pathname}${search}${hash}`) navigate(next);
  };
  ...
  return <header style={{ position: 'sticky', top: 0, zIndex, background: 'rgba(255,255,255,.94)', backdropFilter: 'blur(10px)', borderBottom: '1px solid #E7EDF3' }}>
    <div style={{ maxWidth, margin: '0 auto', padding: '0 24px', height: 70, display: 'flex', alignItems: 'center', gap: 28 }}>
      <Link to={to('/')} style={{ display: 'flex', alignItems: 'center', flexShrink: 0, background: 'transparent' }} aria-label="Deals68.com">
        <img
          src={logoSrc}
      ...
      <nav className="d68-nav" style={{ display: 'flex', alignItems: 'center', gap: 26, marginLeft: 8, fontSize: 15, fontWeight: 500, color: '#14315A' }} aria-label="Main navigation">
        <NavLink to={to('/businesses')} style={navLinkStyle}><span className="l-vi">Doanh nghiệp</span><span className="l-en">Businesses</span></NavLink>
        <NavLink to={to('/investors')} style={navLinkStyle}><span className="l-vi">Nhà đầu tư</span><span className="l-en">Investors</span></NavLink>
        <NavLink to={to('/valuation')} style={navLinkStyle}><span className="l-vi">Định giá</span><span className="l-en">Valuation</span></NavLink>
        <NavLink to={to('/pricing')} style={navLinkStyle}><span className="l-vi">Bảng giá</span><span className="l-en">Pricing</span></NavLink>
      </nav>
      ...
      <div className="d68-mdrawer" style={{ position: 'absolute', top: 70, left: 0, right: 0, flexDirection: 'column', gap: 2, background: '#fff', borderTop: '1px solid #E7EDF3', boxShadow: '0 18px 34px rgba(15,42,74,.14)', padding: '12px 20px 18px', maxHeight: 'calc(100vh - 70px)', overflowY: 'auto' }}>
        <Link to={to('/businesses')} style={{ padding: '13px 6px', fontSize: 16, fontWeight: 600, color: '#14315A', borderBottom: '1px solid #F1F5F9' }}><span className="l-vi">Doanh nghiệp</span><span className="l-en">Businesses</span></Link>
        <Link to={to('/investors')} style={{ padding: '13px 6px', fontSize: 16, fontWeight: 600, color: '#14315A', borderBottom: '1px solid #F1F5F9' }}><span className="l-vi">Nhà đầu tư</span><span className="l-en">Investors</span></Link>
        <Link to={to('/valuation')} style={{ padding: '13px 6px', fontSize: 16, fontWeight: 600, color: '#14315A', borderBottom: '1px solid #F1F5F9' }}><span className="l-vi">Định giá</span><span className="l-en">Valuation</span></Link>
        <Link to={to('/pricing')} style={{ padding: '13px 6px', fontSize: 16, fontWeight: 600, color: '#14315A', borderBottom: '1px solid #F1F5F9' }}><span className="l-vi">Bảng giá</span><span className="l-en">Pricing</span></Link>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '14px 6px 8px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #E2E8F0', borderRadius: 999, overflow: 'hidden', fontSize: 13, fontWeight: 700 }}><button onClick={() => switchLang('vi')} style={langBtnStyle(lang === 'vi')}>VI</button><button onClick={() => switchLang('en')} style={langBtnStyle(lang === 'en')}>EN</button></div>
        </div>
        <Link to={to('/login')} style={{ padding: '13px 6px', fontSize: 16, fontWeight: 700, color: '#14315A' }}><span className="l-vi">Đăng nhập</span><span className="l-en">Log in</span></Link>
      ...
      <div className="d68-hdr-actions" style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #E2E8F0', borderRadius: 999, overflow: 'hidden', fontSize: 13, fontWeight: 700 }}><button onClick={() => switchLang('vi')} style={langBtnStyle(lang === 'vi')}>VI</button><button onClick={() => switchLang('en')} style={langBtnStyle(lang === 'en')}>EN</button></div>
        <Link to={to('/login')} style={{ fontSize: 15, fontWeight: 600, color: '#14315A', whiteSpace: 'nowrap' }}><span className="l-vi">Đăng nhập</span><span className="l-en">Log in</span></Link>
