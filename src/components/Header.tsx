...
export default function Header({ lang, setLang }: { lang: Lang; setLang: (l: Lang) => void }) {
  ...
  return <header style={{ position: 'sticky', top: 0, zIndex, background: 'rgba(255,255,255,.94)', backdropFilter: 'blur(10px)', borderBottom: '1px solid #E7EDF3' }}>
    <div style={{ maxWidth, margin: '0 auto', padding: '0 24px', height: 70, display: 'flex', alignItems: 'center', gap: 28 }}>
      <Link to="/" style={{ display: 'flex', alignItems: 'center', flexShrink: 0 }} aria-label="Deals68.com">
        <img src="/assets/logo-beta.png" alt="Deals68.com" style={{ height: 34, width: 'auto', display: 'block' }} />
      </Link>
      ...
      <nav className="d68-nav" style={{ display: 'flex', alignItems: 'center', gap: 26, marginLeft: 8, fontSize: 15, fontWeight: 500, color: '#14315A' }} aria-label="Main navigation">
        ...
        <NavLink to="/register/advisor" style={navLinkStyle}><span className="l-vi">Cố vấn</span><span className="l-en">Advisors</span></NavLink>
      ...
      <div className="d68-mdrawer" style={{ position: 'absolute', top: 70, left: 0, right: 0, flexDirection: 'column', gap: 2, background: '#fff', borderTop: '1px solid #E7EDF3', boxShadow: '0 18px 34px rgba(15,42,74,.14)', padding: '12px 20px 18px', maxHeight: 'calc(100vh - 70px)', overflowY: 'auto' }}>
        ...
        <Link to="/register/advisor" style={{ padding: '13px 6px', fontSize: 16, fontWeight: 600, color: '#14315A', borderBottom: '1px solid #F1F5F9' }}><span className="l-vi">Cố vấn</span><span className="l-en">Advisors</span></Link>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, padding: '14px 6px 8px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', border: '1px solid #E2E8F0', borderRadius: 999, overflow: 'hidden', fontSize: 13, fontWeight: 700 }}>
            <button onClick={() => setLang('vi')} style={langBtnStyle(lang === 'vi')}>VI</button>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 8 }}>
          ...
          <Link to="/register/advisor" style={{ textAlign: 'center', border: '1px solid #E2E8F0', color: '#14315A', fontWeight: 700, fontSize: 15, padding: 13, borderRadius: 10 }}><span className="l-vi">Đăng ký Cố vấn</span><span className="l-en">Register as Advisor</span></Link>
        </div>
      </div>
      ...
      <div className="d68-hdr-actions" style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 14, flexShrink: 0 }}>
        ...
        <div className="d68-reg-dd" style={{ position: 'relative' }}>
          <button style={{ display: 'inline-flex', alignItems: 'center', gap: 6, background: '#1BADEA', color: '#fff', fontWeight: 700, fontSize: 15, padding: '11px 20px', borderRadius: 10, boxShadow: '0 6px 16px rgba(27,173,234,.28)', whiteSpace: 'nowrap', border: 'none', cursor: 'pointer' }}>
            <span className="l-vi">Đăng ký</span><span className="l-en">Register</span> <span style={{ fontSize: 11 }}>▾</span>
          </button>
          <div className="d68-reg-menu" style={{ display: 'none', position: 'absolute', top: '100%', right: 0, paddingTop: 8, minWidth: 230, flexDirection: 'column', zIndex: 80 }}>
            <div style={{ background: '#fff', border: '1px solid #E7EDF3', borderRadius: 12, boxShadow: '0 14px 34px rgba(15,42,74,.16)', padding: 8, display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Link to="/register/advisor" style={{ padding: '11px 14px', borderRadius: 8, fontSize: 14.5, fontWeight: 600, color: '#14315A' }}><span className="l-vi">Đăng ký Cố vấn</span><span className="l-en">Register as Advisor</span></Link>
            </div>
          </div>
        </div>
      </div>
