...
export default function Header({ lang, setLang }: { lang: Lang; setLang: (l: Lang) => void }) {
  ...
  const registerLinks = <>
    ...
    <Link to="/register/market-partner">{T(lang, 'Đăng ký Market Partner', 'Register as Market Partner')}</Link>
  </>;

  return <header className="d68-ref-header">
    <div className="d68-ref-header__inner">
      <Link to="/" className="d68-ref-logo"><img src="/assets/logo-beta.png" alt="Deals68.com" /></Link>
      <nav className="d68-nav d68-ref-nav">
        ...
        <NavLink to="/dashboard/advisor" style={({isActive}) => navStyle(isActive)}>{T(lang, 'Cố vấn', 'Advisors')}</NavLink>
      ...
      <input id="d68burger" className="d68-burger-cb" type="checkbox" aria-label="Open menu" />
      ...
      <div className="d68-mdrawer">
        ...
        {profile ? <>
          <button onClick={() => navigate(dashboardPath)}>{profile.role === 'admin' ? 'Admin' : 'Dashboard'}</button>
          ...
          <Link className="d68-mdrawer__ghost" to="/register/market-partner">Market Partner</Link>
        </>}
      </div>
      ...
      <div className="d68-hdr-actions d68-ref-actions">
        <div className="d68-ref-lang"><button style={langBtn(lang === 'vi')} onClick={() => setLang('vi')}>VI</button><button style={langBtn(lang === 'en')} onClick={() => setLang('en')}>EN</button></div>
        {profile ? <>
          <button className="d68-ref-login" onClick={() => navigate(dashboardPath)}>{profile.role === 'admin' ? 'Admin' : 'Dashboard'}</button>
