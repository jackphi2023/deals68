...
export type BannerPlacement = 'home_hero' | 'home_promotion' | 'listing_promotion';
...
export function bannerIsActive(row: SiteBanner) {
  ...
  return row.active !== false && (!row.starts_at || row.starts_at <= today) && (!row.ends_at || row.ends_at >= today);
}

export async function listSiteBanners(placement: BannerPlacement, lang: Lang, admin = false): Promise<SiteBanner[]> {
  let q: any = supabase
    .from('site_banners')
    .select('*')
  ...
  if (!admin) {
    const today = todayIso();
    q = q.eq('active', true).lte('starts_at', today).or(`ends_at.is.null,ends_at.gte.${today}`);
  }
...
export async function uploadSiteBannerImage(file: File, placement: BannerPlacement) {
  ...
  const { error: upErr } = await supabase.storage.from('site-banners').upload(path, file, { upsert: false });
