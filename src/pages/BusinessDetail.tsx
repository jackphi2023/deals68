...
function normalizeSimilar(row: any, lang: Lang): SimilarDeal | null {
  ...
  return { id: String(row.id || slug), slug, title, industry: labelIndustry(primaryIndustry(row.industry), lang), city: labelLocation(row.city || row.country_iso2, lang), revenue: money(lang, row.revenue_2025, row.revenue_currency || 'VND'), ask: money(lang, row.ask_amount, row.ask_currency || row.revenue_currency || 'VND'), image: row.image_url || row.hero_image_url || null };
}
...
export default function BusinessDetail({ lang }: { lang: Lang }) {
  ...
  return <main className="d68-business-detail-page">
    <div className="d68-detail-shell">
      <nav className="d68-detail-breadcrumb"><Link to="/">{T(lang, 'Trang chủ', 'Home')}</Link><span>›</span><Link to="/businesses">{T(lang, 'Doanh nghiệp', 'Businesses')}</Link><span>›</span><Link to={`/businesses?industry=${encodeURIComponent(industry)}`}>{industry}</Link><span>›</span><b>{business.public_code || business.slug}</b></nav>
