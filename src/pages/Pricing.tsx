...
export default function Pricing({ lang }: { lang: Lang }) {
  ...
  const planName = role === 'business' ? (bizPlan === 'featured' ? T(lang, 'Gói Ưu tiên', 'Priority package') : T(lang, 'Gói Thường', 'Regular package')) : roleLabel(lang, role);
  const termWeeks = role === 'business' ? units : units * 4;

  const planCards = [
    { key: 'business' as Role, icon: '🏢', titleVi: 'Doanh nghiệp', titleEn: 'Business', descVi: 'Đăng hồ sơ gọi vốn, bán doanh nghiệp, vay vốn hoặc tìm đối tác chiến lược.', descEn: 'List a fundraise, business sale, loan or strategic partnership profile.', price: money(CFG.business.vn, 'VND'), perVi: 'tuần', perEn: 'week', popular: true, featuresVi: [T(lang, 'Hồ sơ ẩn danh công khai', 'Public anonymous profile'), T(lang, 'Nhận quan tâm từ nhà đầu tư', 'Receive investor interest'), T(lang, 'Dashboard cập nhật số liệu', 'Data update dashboard'), T(lang, 'Tài liệu mở sau kết nối', 'Documents unlock after connection')] },
    { key: 'investor' as Role, icon: '📈', titleVi: 'Nhà đầu tư', titleEn: 'Investor', descVi: 'Tìm doanh nghiệp phù hợp, lưu deal và gửi yêu cầu kết nối/data room.', descEn: 'Find matching businesses, save deals and request connection/data room.', price: money(CFG.investor.vn, 'VND'), perVi: 'tháng', perEn: 'month', popular: false, featuresVi: [T(lang, 'Tìm kiếm thương vụ', 'Search for deals'), T(lang, 'Xem Business Quality Score', 'View Business Quality Score'), T(lang, 'Lưu doanh nghiệp quan tâm', 'Save businesses of interest'), T(lang, 'Xem chi tiết hồ sơ và Yêu cầu cung cấp tài liệu', 'View detailed profiles and request supporting documents')] },
    { key: 'advisor' as Role, icon: '🤝', titleVi: 'Cố vấn', titleEn: 'Advisor', descVi: 'Đại diện nhiều thương vụ, quản lý khách hàng và hỗ trợ kết nối.', descEn: 'Represent multiple mandates, manage clients and support matching.', price: money(CFG.advisor.vn, 'VND'), perVi: 'tháng', perEn: 'month', popular: false, featuresVi: [T(lang, 'Quản lý nhiều hồ sơ', 'Manage multiple profiles'), T(lang, 'Tiếp cận nhiều nhà đầu tư hơn', 'Reach more investors'), T(lang, 'Kết nối Nhà đầu tư và Doanh nghiệp', 'Connect investors and businesses'), T(lang, 'Hồ sơ cố vấn', 'Advisor profile')] }
  ];
  const tiers = [
