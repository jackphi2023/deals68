...
function DocumentRow({ lang, d, renameFile, downloadFile, deleteFile }: any) {
  ...
  return <div className="d68-dashboard-row d68-document-row">
    ...
    />
    <button type="button" onClick={save} disabled={saving || downloading} className="d68-dashboard-btn light">{saving ? T(lang,'Đang lưu','Saving') : T(lang,'Lưu tên','Save name')}</button>
    <button type="button" onClick={download} disabled={saving || downloading} className="d68-dashboard-btn light">{downloading ? T(lang,'Đang tải','Downloading') : T(lang,'Tải tài liệu','Download')}</button>
    <button type="button" onClick={() => deleteFile(d)} disabled={saving || downloading} className="d68-dashboard-btn red">{T(lang,'Xóa','Delete')}</button>
