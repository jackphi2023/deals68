...
function viIndustry(raw: any) { const v = String(raw || '').toLowerCase(); if (v.includes('health')) return 'Y tế & Sức khỏe'; if (v.includes('beauty') || v.includes('personal')) return 'Làm đẹp & Chăm sóc cá nhân'; if (v.includes('retail')) return 'Bán lẻ'; if (v.includes('manufact')) return 'Sản xuất'; if (v.includes('tech')) return 'Công nghệ'; if (v.includes('real')) return 'Bất động sản'; if (v.includes('logistics')) return 'Logistics'; if (v.includes('education')) return 'Giáo dục'; if (v.includes('energy')) return 'Năng lượng'; if (v.includes('seafood')) return 'Thủy sản & Xuất khẩu'; return raw || 'F&B'; }
function viDealType(raw: any) { const v = String(raw || '').toLowerCase(); if (v.includes('fund') || v.includes('primary') || v.includes('share') || v.includes('equity')) return 'Gọi vốn'; if (v.includes('loan') || v.includes('debt')) return 'Vay vốn'; if (v.includes('asset')) return 'Chuyển nhượng tài sản'; if (v.includes('sale') || v.includes('m&a') || v.includes('transfer') || v.includes('acquisition')) return 'M&A / Chuyển nhượng'; if (v.includes('jv') || v.includes('partner')) return 'JV / Đối tác'; return raw || 'Gọi vốn'; }
