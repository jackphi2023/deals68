...
function DealCard({ d, lang, view, tintIndex }: { d: Deal; lang: Lang; view: ViewMode; tintIndex: number }) {
  ...
  return (
    <Link to={`/businesses/${d.slug}`} className={`d68-business-card${view === 'list' ? ' d68-business-card--list' : ''}`}>
  ...
  return (
    <main className="d68-businesses-page">
      <section className="d68-businesses-title">
        <div className="d68-businesses-breadcrumb"><Link to="/">{T(lang, 'Trang chủ', 'Home')}</Link> › <b>{T(lang, 'Doanh nghiệp', 'Businesses')}</b></div>
