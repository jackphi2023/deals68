...
export default function Valuation({ lang }: { lang: Lang }) {
  ...
  function apply() { setApplied({ country, industry, revenue: Number(revenue || 0), margin: Number(margin || 0), growth: Number(growth || 0), founded: Number(founded || 0) }); }

  return <main className="d68-valuation-page">
    <section className="d68-val-hero"><div><span>{T(lang, 'Miễn phí · Không cần đăng nhập', 'Free · No login required')}</span><h1>{T(lang, 'Định giá sơ bộ doanh nghiệp của bạn', 'Estimate your business valuation')}</h1><p>{T(lang, 'Nhận khoảng định giá tham khảo theo ngành, quốc gia và tài chính — dựa trên bội số benchmark do Deals68 cấu hình.', 'Get an indicative valuation range by industry, country and financials — using Deals68 configurable benchmark multiples.')}</p></div></section>
    <section className="d68-val-wrap"><div className="d68-val-cols"><article className="d68-val-card"><h2>{T(lang, 'Thông tin doanh nghiệp', 'Business details')}</h2><div className="d68-val-form"><label>{T(lang, 'Quốc gia', 'Country')}<select value={country} onChange={(e) => setCountry(e.target.value)}>{COUNTRIES.map((c) => <option key={c.value}>{c.value}</option>)}</select></label><label>{T(lang, 'Ngành', 'Industry')}<select value={industry} onChange={(e) => setIndustry(e.target.value)}>{INDUSTRIES.map((i) => <option key={i.key} value={i.key}>{T(lang, i.vi, i.en)}</option>)}</select></label><label>{T(lang, 'Doanh thu năm gần nhất', 'Latest annual revenue')}<input type="number" value={revenue} onChange={(e) => setRevenue(Number(e.target.value))}/><small>{country === 'Vietnam' ? 'tỷ VND' : 'million USD'}</small></label><label>EBITDA margin (%)<input type="number" value={margin} onChange={(e) => setMargin(Number(e.target.value))}/></label><label>{T(lang, 'Tăng trưởng doanh thu (%)', 'Revenue growth (%)')}<input type="number" value={growth} onChange={(e) => setGrowth(Number(e.target.value))}/></label><label>{T(lang, 'Năm thành lập', 'Founded year')}<input type="number" value={founded} onChange={(e) => setFounded(Number(e.target.value))}/></label></div><button type="button" onClick={apply}>{T(lang, 'Cập nhật định giá', 'Update valuation')}</button></article><aside className="d68-val-result"><span>{result.conf}</span><h2>{fmt(result.low, result.cur)} – {fmt(result.high, result.cur)}</h2><p>{T(lang, 'Khoảng định giá tham khảo, không phải tư vấn đầu tư hay cam kết giao dịch.', 'Indicative valuation range, not investment advice or a transaction commitment.')}</p><div><Row a="Revenue multiple" b={result.revMultiple}/><Row a="EBITDA multiple" b={result.ebitdaMultiple}/><Row a={T(lang,'Ngành','Industry')} b={T(lang,result.ind.vi,result.ind.en)}/><Row a={T(lang,'Quốc gia','Country')} b={result.country.value}/></div><Link to={toLocalizedPath('/register/business', lang)}>{T(lang, 'Đăng hồ sơ doanh nghiệp', 'List your business')} →</Link></aside></div></section>
    <section className="d68-val-method"><h2>{T(lang, 'Cách Deals68 tính tham khảo', 'How Deals68 estimates')}</h2><div><article><b>1</b><h3>{T(lang,'Bội số ngành','Industry multiples')}</h3><p>{T(lang,'Dùng bội số doanh thu và EBITDA cơ sở theo ngành.','Uses base revenue and EBITDA multiples by industry.')}</p></article><article><b>2</b><h3>{T(lang,'Điều chỉnh thị trường','Market adjustment')}</h3><p>{T(lang,'Điều chỉnh theo quốc gia, tăng trưởng và biên lợi nhuận.','Adjusted by country, growth and margin.')}</p></article><article><b>3</b><h3>{T(lang,'Độ tin cậy','Confidence')}</h3><p>{T(lang,'Độ tin cậy cao hơn khi có đủ doanh thu, EBITDA, tăng trưởng và lịch sử hoạt động.','Higher confidence when revenue, EBITDA, growth and operating history are available.')}</p></article></div></section>
  </main>;
}
