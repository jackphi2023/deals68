...
export default function Valuation({ lang }: { lang: Lang }) {
  ...
  return <main className="d68-valuation-page">
    <section className="d68-val-hero"><div><span>{T(lang, 'Miễn phí · Không cần đăng nhập', 'Free · No login required')}</span><h1>{T(lang, 'Định giá sơ bộ doanh nghiệp của bạn', 'Estimate your business valuation')}</h1><p>{T(lang, 'Nhận khoảng định giá tham khảo theo quốc gia, ngành, doanh thu năm gần nhất, biên lợi nhuận EBITDA và tăng trưởng doanh thu. Hệ số do Admin Deals68 cấu hình và có thể cập nhật theo dữ liệu thị trường.', 'Get an indicative benchmark by country, industry, latest annual revenue, EBITDA margin and revenue growth. Multiples are configured by Deals68 Admin and can be updated with market data.')}</p></div></section>
