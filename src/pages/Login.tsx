    ...
    <main className="d68-auth-page">
      <section className="d68-auth-card">
        <div className="d68-auth-head">
          <span>{isAdminLogin ? 'Admin' : 'Deals68'}</span>
          <h1>{isAdminLogin ? 'Admin Login' : T(lang, 'Đăng nhập Deals68', 'Log in to Deals68')}</h1>
          <p>
            {isAdminLogin
        ...
        <form onSubmit={submit} className="d68-auth-form">
          ...
          <div className="d68-auth-row">
            <Link to={toLocalizedPath(`/forgot-password?role=${role}`, lang)}>
          ...
          <button className="d68-auth-submit" disabled={loading}>
            {loading ? T(lang, 'Đang đăng nhập...', 'Logging in...') : T(lang, 'Đăng nhập', 'Log in')}
          </button>
        </form>
        ...
        {!isAdminLogin && currentRole.register ? (
          <p className="d68-auth-bottom">
            ...
            <Link to={toLocalizedPath(currentRole.register, lang)}>
              {T(lang, `Đăng ký ${currentRole.vi}`, 'Register')}
            </Link>
          </p>
        ) : null}
      </section>
