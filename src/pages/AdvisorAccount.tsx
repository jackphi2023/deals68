    ...
    <main className="d68-advisor-account-page">
      <section className="d68-advisor-account-shell">
        <header className="d68-advisor-account-header">
          <div><span>Deals68 Advisor</span><h1>{T(lang, 'Danh mục khách hàng và Business intake', 'Client portfolio and Business intake')}</h1><p>{T(lang, 'Phiên 7 bổ sung thẩm định từng bằng chứng, file thay thế và tái thẩm định authority theo thời hạn. Business context vẫn chỉ đọc và tự đóng nếu authority không còn verified/hợp lệ.', 'Session 7 adds per-evidence validation, replacement files and expiry-driven authority re-review. Business context remains read only and automatically closes when authority is no longer verified/current.')}</p></div>
        ...
        {!loading && advisor ? <>
          <div className={`d68-advisor-account-banner ${approved ? 'approved' : advisor.status === 'rejected' || advisor.status === 'suspended' ? 'blocked' : 'pending'}`}>
            ...
            <span>{approved ? T(lang, 'Bạn có thể gửi Business mới, nộp hoặc thay thế bằng chứng authority và chỉ thấy các Business có assignment của mình. Tài liệu authority không tạo ownership hoặc quyền chỉnh sửa.', 'You may submit new Businesses, provide or replace authority evidence and only see Businesses linked to your assignments. Authority evidence creates no ownership or edit access.') : T(lang, 'Email đã xác thực nhưng Admin vẫn cần kích hoạt và xác minh hồ sơ.', 'Email is verified, but Admin activation and verification are still required.')}</span>
          </div>
            ...
            <section className="d68-advisor-workspace">
              <aside className="d68-advisor-portfolio-list">
                ...
                {portfolio.map((item) => <article key={item.assignment_id} className={`d68-advisor-assignment ${businessId === item.business_id ? 'selected' : ''}`}>
                  ...
                  {item.status === 'active' && !item.can_open_context ? <p className="d68-advisor-warning">{T(lang, 'Business context đang bị khóa vì scope hoặc authority không còn hợp lệ; kiểm tra trạng thái tái thẩm định bên dưới.', 'Business context is locked because scope or authority is not currently valid; check the re-review status below.')}</p> : null}
                </article>)}
              </aside>
              ...
              <section className="d68-advisor-context-panel">
                ...
                {!contextLoading && context ? <>
                  ...
                  <section className="d68-advisor-context-access"><h3>{T(lang, 'Phạm vi đang mở', 'Current access')}</h3><div>{context.assignment.permissions.map((scope) => <span key={scope}>{scope}</span>)}</div><p>{T(lang, 'Phiên 7 vẫn chỉ trả về nhận diện và trạng thái Business cơ bản trong context. Evidence authority nằm ở kho riêng; không có dữ liệu tài chính, file Business riêng tư, proposal, yêu cầu dữ liệu, thanh toán, báo cáo hoặc mutation.', 'Session 7 still returns only basic Business identity and status in context. Authority evidence is isolated in a private store; financials, private Business files, proposals, data requests, payments, reports and mutations remain unavailable.')}</p></section>
                </> : null}
              </section>
            </section>
          </> : null}
          ...
          <section className="d68-advisor-account-panel d68-advisor-session-boundary"><h2>{T(lang, 'Ranh giới Phiên 7', 'Session 7 boundary')}</h2><ul><li>{T(lang, 'Admin có thể đánh dấu evidence Hợp lệ / Chưa đủ / Không hợp lệ và yêu cầu file thay thế. File đã nộp vẫn bất biến; replacement chỉ supersede về mặt review.', 'Admin may classify evidence as Valid / Insufficient / Invalid and request replacement. Submitted files remain immutable; replacement only supersedes them in review state.')}</li><li>{T(lang, 'Khi authority cần tái thẩm định, nó quay về pending_review và Business context tự khóa cho đến khi Admin tái xác minh. Việc tái duyệt vẫn chỉ giữ scope profile.', 'When authority enters re-review it returns to pending_review and Business context automatically locks until Admin re-verifies it. Re-approval remains profile-only.')}</li><li>{T(lang, 'Business vẫn ownerless/draft/non-public trong intake; không có quyền sửa Business, payment, file Business, proposal, data request hoặc report.', 'The intake Business remains ownerless/draft/non-public; no Business editing, payments, Business files, proposals, data requests or reports are enabled.')}</li></ul><p>{T(lang, 'Cần hỗ trợ?', 'Need support?')} <Link to={toLocalizedPath('/contact', lang)}>{T(lang, 'Liên hệ Deals68', 'Contact Deals68')}</Link>.</p></section>
        </> : null}
      </section>
    </main>
  );
