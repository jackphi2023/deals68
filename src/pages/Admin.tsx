...
function isPendingBusinessFile(row: Row) {
  ...
  return row.public_visible !== true || privacy.includes('locked') || note.includes('review') || note.includes('pending') || note.includes('user');
}
function isPendingBusinessImage(row: Row) {
  const note = String(row.admin_note || '').toLowerCase();
  return row.public_visible !== true || row.is_sanitized !== true || note.includes('review') || note.includes('pending') || note.includes('user');
}
