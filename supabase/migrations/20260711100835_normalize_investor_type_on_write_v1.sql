-- Keep future Investor registrations and Admin edits aligned with the canonical type value.
...
declare
  ...
  v_criteria jsonb;
begin
  if lower(trim(coalesce(new.type, ''))) in (
    'individual/angel',
    'individual / angel',
    'individual',
    'angel'
  ) then
    new.type := 'Nhà đầu tư cá nhân';
  end if;
...
before update of type, industries, deal_types, criteria on public.investors
for each row
