...
comment on column public.request_data.granted_scopes is
  'Scopes approved by the Business. Empty on legacy rows is normalized by the access trigger.';
...
declare
  ...
  action_name text;
begin
  proposal_row := case when tg_op = 'DELETE' then old else new end;
  ...
  if actor_uuid is not null then
    insert into public.audit_logs (
      ...
      jsonb_build_object(
        ...
        'proposal_status', case when tg_op = 'DELETE' then 'deleted' else proposal_row.status::text end,
...
declare
  ...
  action_name text;
begin
  request_row := case when tg_op = 'DELETE' then old else new end;
  ...
  if tg_op <> 'DELETE' and request_row.status::text = 'fulfilled' then
    ...
    should_revoke := tg_op = 'DELETE'
      or (tg_op = 'UPDATE' and old.status::text = 'fulfilled' and new.status::text <> 'fulfilled')
      or (tg_op <> 'DELETE' and request_row.status::text = 'rejected');
  ...
  if actor_uuid is not null and action_name is not null then
    insert into public.audit_logs (
      ...
      jsonb_build_object(
        ...
        'request_status', case when tg_op = 'DELETE' then 'deleted' else request_row.status::text end,
...
-- Preserve the newest open request per Business/Investor pair and close legacy duplicates.
...
set status = 'rejected'::public.request_status,
    response_note = coalesce(r.response_note, 'Superseded by a newer pending request during access-contract migration.'),
...
begin
  ...
  if public.is_admin() or service_actor or business_row.owner_id = actor_uuid then
    return jsonb_build_object(
      ...
      'has_dataroom', true,
      'sources', jsonb_build_array(jsonb_build_object('source_type', case when public.is_admin() or service_actor then 'admin' else 'owner' end)),
      'expires_at', null,
  ...
  select
    coalesce(array_agg(distinct scope_value order by scope_value), '{}'::text[]),
    coalesce(jsonb_agg(
    ...
    count(distinct grant_row.id)::integer,
    bool_or(grant_row.expires_at is null),
...
begin
  select r.*, b.owner_id
  into request_row, business_owner
  from public.request_data r
  ...
  return jsonb_build_object(
    ...
    'status', case when normalized_decision in ('reject', 'rejected', 'decline', 'declined') then 'rejected' else 'fulfilled' end,
...
declare
  ...
  snapshot jsonb;
begin
  select grant_record.*, b.owner_id
  into grant_row, business_owner
  from public.business_financial_access_grants grant_record
  join public.businesses b on b.id = grant_record.business_id
  ...
  set status = 'revoked',
      revoked_by = actor_uuid,
      revoked_at = now(),
      revoke_reason = coalesce(nullif(left(trim(coalesce(p_reason, '')), 1000), ''), 'revoked_by_business'),
      updated_at = now()
  where id = p_grant_id;
...
-- Backfill Proposal summary access. Declined Proposal rows intentionally receive no grant.
insert into public.business_financial_access_grants (
  business_id,
  investor_id,
  source_type,
...
-- Backfill fulfilled legacy requests as financial summary + detail. Dataroom is never inferred.
insert into public.business_financial_access_grants (
  business_id,
  investor_id,
  source_type,
...
revoke all on table public.business_financial_access_grants from public, anon, authenticated;
...
insert into public.audit_logs (
  ...
  jsonb_build_object(
    'proposal_grants', (
      select count(*) from public.business_financial_access_grants where source_type = 'proposal'
    ),
    'data_request_grants', (
      select count(*) from public.business_financial_access_grants where source_type = 'data_request'
    ),
