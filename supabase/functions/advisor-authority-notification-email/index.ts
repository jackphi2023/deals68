...
Deno.serve(async (req: Request) => {
  ...
  // verify_jwt=true at the Edge gateway validates the caller JWT. That public JWT is not
  // the scheduler authorization. A separate high-entropy token is stored only in Vault;
  // the service-only authorization RPC compares it before any queue claim is allowed.
  ...
  if (authError || authorized !== true) {
    return json(403, { ok: false, error: 'SCHEDULER_TOKEN_INVALID' });
  }
  ...
  for (const job of jobs) {
    ...
    try {
      ...
      const { error: completeError } = await serviceClient.rpc('d68_notification_worker_complete_v1', {
        p_job_id: job.job_id, p_success: true, p_provider: delivery.provider, p_provider_message_id: delivery.messageId, p_error: null,
      });
      ...
      const { error: failError } = await serviceClient.rpc('d68_notification_worker_complete_v1', {
        p_job_id: job.job_id, p_success: false, p_provider: provider || null, p_provider_message_id: null, p_error: message || 'EMAIL_DELIVERY_FAILED',
      });
