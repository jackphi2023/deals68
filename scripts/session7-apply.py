...
if 'const transactionInfo = useMemo<TransactionInfoRow[]>' not in source:
  ...
  const transactionInfo = useMemo<TransactionInfoRow[]>(() => {
    ...
    replace_once(marker, insert, 'transaction info memo')
...
if 'd68-detail-transaction-info' not in source:
    ...
    replace_once(marker, insert, 'transaction info section')
...
if owner_old in source:
    replace_once(owner_old, owner_new, 'owner view badge')
...
if bqs_start in source:
    ...
    block = block.replace(
        bqs_start,
        '          <section className="d68-detail-card d68-detail-card--bqs">',
        1,
    )
...
subprocess.run(['node', 'scripts/deals68-business-detail-assets-transaction-check.mjs'], check=True)
...
if (fs.existsSync('scripts/session7-apply.py')) {
  execFileSync('python', ['scripts/session7-apply.py'], { stdio: 'inherit' });
}
"""
if hook_block not in hook_source:
    raise SystemExit('Session 7 hook block missing')
hook_path.write_text(hook_source.replace(hook_block, '', 1), encoding='utf-8')

apply_path = Path('scripts/session7-apply.py')
