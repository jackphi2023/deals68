...
assert.ok(/for insert\s+to authenticated[\s\S]*can_advisor_upload_authority_evidence/i.test(migration), 'Storage INSERT must use allocation helper');
assert.ok(/for select\s+to authenticated[\s\S]*can_read_advisor_authority_evidence/i.test(migration), 'Storage SELECT must use owner/Admin helper');
assert.equal(/advisor authority evidence[\s\S]*for update/i.test(migration), false, 'Session 6 must not create Storage UPDATE policy');
assert.equal(/advisor authority evidence[\s\S]*for delete/i.test(migration), false, 'Session 6 must not create Storage DELETE policy');
...
assert.equal(/create\s+policy[\s\S]*on\s+public\.businesses/i.test(migration), false, 'Session 6 must not change Business RLS');
