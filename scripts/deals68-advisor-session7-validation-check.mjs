...
assert.ok(/d68_get_my_authority_review_v[234]/.test(advisorLib), 'Advisor review client must use Session 7 v2 or a later server wrapper');
...
assert.ok(/Ranh giới Phiên (7|8|9)/.test(advisorPage) || /Session (7|8|9) boundary/.test(advisorPage), 'Advisor dashboard must expose Session 7 boundary or a later inherited boundary');

assert.ok(/d68_admin_list_advisor_business_intakes_v[345]/.test(adminLib), 'Admin queue must use Session 7 v3 or a later server wrapper');
...
assert.ok(/Ranh giới Phiên (7|8|9)/.test(adminPage), 'Admin UI must state Session 7 boundary or a later inherited boundary');
