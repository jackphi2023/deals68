...
assert.ok(advisorLib.includes("d68_get_my_authority_review_v2"), 'Advisor review client must use Session 7 v2 read surface');
...
assert.ok(advisorPage.includes('Ranh giới Phiên 7'), 'Advisor dashboard must expose Session 7 boundary');

assert.ok(adminLib.includes('d68_admin_list_advisor_business_intakes_v3'), 'Admin queue must use Session 7 v3 wrapper');
...
assert.ok(adminPage.includes('Ranh giới Phiên 7'), 'Admin UI must state Session 7 boundary');
