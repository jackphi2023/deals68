...
(function (global) {
  ...
  // Commission tier by number of paid conversions: <5 = 10%, 5–14 = 15%, 15+ = 20%.
  function affiliateTier(conversions) {
    if (conversions >= 15) return { rate: 0.20, name: 'Gold', nameVi: 'Vàng' };
    if (conversions >= 5) return { rate: 0.15, name: 'Silver', nameVi: 'Bạc' };
    return { rate: 0.10, name: 'Starter', nameVi: 'Khởi đầu' };
  }
  ...
  function getAffiliateAccount(id) {
    if (id === 'aff' || !id) {
      ...
      return { id: 'aff', name: demo.name, email: demo.email, code: demo.code, payoutMethod: demo.payoutMethod, accountStatus: 'active', dashboardLoginEnabled: true, paid: true };
    }
  ...
  function registerAffiliate(data) {
    var code = 'AFF-' + Math.floor(1000 + Math.random() * 9000);
    var id = 'aff_' + code.toLowerCase().replace(/[^a-z0-9]/g, '');
    var state = {
      id: id, code: code, name: data.name, email: data.email,
    ...
    logAudit('affiliate_registered', id + ' — pending admin approval');
    return state;
  }
